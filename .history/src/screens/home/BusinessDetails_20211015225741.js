...
const BusinessDetails = ({route}) => {
    ...
    const onCardPress = (id, name, favorite) => {
        navigation.navigate('EventsStack',{screen:'EventDetails',params:{EventID: id, EventName:name, favorite}})
    }
    ...
    const sendMessage = () => {
        ...
        firebase.firestore().collection('chats').doc(`${user.ModifiedBy}_${person.ModifiedBy}`).set({
            lastMessage: new Date().getTime(),
            lastMessageText: `${user.FirstName} ${user.LastName} have opened chat`,
            lastMessageBy: ``,
            fromID: user.ModifiedBy,
            fromName: `${user.FirstName} ${user.LastName}`,
            fromPhoto:user.Photo,
            fromSqlID:user.idUser,
            toID: person.ModifiedBy,
            toPhoto:person.Photo,
            toSqlID:person.idUser,
            toName: `${person.FirstName} ${person.LastName}`,
            read: false,
            readBy: [],
        }).then(()=>{
            firebase.firestore().collection('chats').doc(`${user.ModifiedBy}_${person.ModifiedBy}`)
            .collection('messages').add({}).then(()=>this.gotoChat()).catch((err)=>alert(err))
        }).catch((err)=>{
            alert(err)
        })
    }
    ...
    const getImages = () => {
        ...
        businessDetails.images.split(',').map(image=>
            imgArr.push(route.params.imgPath+'/'+image)
        )
        return imgArr;
    }
                ...
                <FlatList
                    ...
                    renderItem={({item,index}) =>
                        <ImageCard
                            ...
                            image={{uri: item.images? ImgPath+"/"+item.images.split(",")[0]:"abc"}}
                            title={item.name}
    ...
    return(
        <Container>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <ButtonRow
                    button1Icon={"call"}
                    button1Text={businessDetails.phone}
                ...
                <View style={[styles.textContainer,{borderBottomColor:colors.border}]}>
                    <Text style={[styles.heading,{fontSize:hp('3%'), color:colors.heading}]}>{toTitleCase(businessDetails.name)}</Text>
